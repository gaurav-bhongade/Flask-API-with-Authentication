...
app = Flask(__name__)
app.config['JWT_SECRET_KEY'] = '1287A2^#&Dsb(@sdvhrtefwet' #token configure hone ke liye sabsepehele kya hona chahiye
...
def validate_user():
    ...
    if req_body.get('USERNAME') == 'admin' and req_body.get('PASSWORD') == 'root123':
        ...
        return json.dumps({"access_token": atoken,"refresh_token": rtoken}) #yahase jaisehitoken mil jayega to kya return kar dunga mai
    else:
        return jsonify('Invalid credentials...!'), 401

#return json.dumps({"ERROR": "Invalid Credentials...!"}) iske alawa jsonify b ek objecthota hai
